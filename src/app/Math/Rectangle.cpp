...
Rectangle::Rectangle(Double c, Double s) : center(c), size(s) {}

Rectangle::Rectangle(Double s) : center(0.0f, 0.0f), size(s) {}

bool Rectangle::contains(Double p) const {
    return !(
...
int Rectangle::quadrant(Double p) const {
    bool x{p.x >= center.x};
    bool y{p.y >= center.y};
